...
In this section we are going to analyze and visualize the relationship between the variables in our data set through using the data visualization package ggplot2.
...
labs(title="                                Counts for Tooth length measurements ",x="Tooth Length",
 caption = "Fig(4)                                                                                                       ")
```
From Fig(4), we can see that the peak of the histogram occurs at length around 26 with 5 teeth occurred at the bin.
...
geom_density(alpha=0.5)+
  labs(title="                Tooth growth based on the Delivery method",x="Tooth Length",y="Density",fill="Delivery Method", caption = "Fig(5)                                                                                    ")
 


```
Fig (5), shows the distribution of tooth length for each dose delivery method.
...
ggplot(ToothGrowth, aes(x = supp, y = len)) + geom_boxplot(aes(fill = supp))+
  labs(title="           Tooth growth based on the dose delivery method",x="Delivery Method",y="Length",fill="Delivery Method", caption = "Fig(6)                                                                                    ")
```

Fig(6) shows the distribution of the tooth growth data based on each dose delivery method. This boxplot shows the minimum, maximum, median, first quartile and third quartile in the data set.
...
ggplot(ToothGrowth,aes(x=len, fill=dose)) + geom_density(alpha=0.3)+
  labs(title="                Tooth growth based on the Dose level",x="Tooth Length",y="Density",fill="Dose  Level ",caption = "Fig(7)                                                                                      ")
 
```

Fig (7), shows the distribution of tooth length for each dose level.
...
ggplot(ToothGrowth, aes(x = supp, y = len)) + geom_boxplot(aes(fill = dose))+
  labs(title="                        Tooth growth based on the dose level",x="Dose Level",y="Length",fill="Dose Level", caption = "Fig(8)                                                                                    ")
```


Fig (8), shows the distribution of the tooth growth data based on each dose level. This boxplot shows the minimum, maximum, median, first quartile and third quartile in the data set.
...
```{r}
ggplot(women,aes(height,weight))+geom_point()+labs(title="                                                  Height vs weight",x="Height in In ",y="Weight in Ibs", caption = "Fig(9)                                                                                                       ")

```

Fig (9), shows us that the relationship between the height and the weight for the American women is linearly.
