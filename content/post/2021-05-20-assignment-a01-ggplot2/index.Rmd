...
ggplot(ToothGrowth, aes(x = supp, y = len)) + geom_boxplot(aes(fill = supp))+
  labs(title="           Tooth growth based on the dose delivery method",x="Tooth Length",y="Density",fill="Delivery Method", caption = "Fig(6)                                                                                    ")
```
...
```
Fig (6), shows the distribution of tooth length for each dose level.



orange juice delivery method results in better tooth growth with the 0.5 and 1 mg/day doses while ascorbic acid delivery method results in better tooth growth with 2 mg/day dose.
